...
public final class CommandMap {
    ...
    public void discordCommandConsole(String command){
        ...
        if(object[0] == null || ((SimpleCommand)object[0]).getExecutorType() == Command.ExecutorType.USER){
            bot.getLogger().warn("Commande inconnue.");
            return;
        }
        try{
            executeDiscordCommand(((SimpleCommand)object[0]), command, (String[])object[1], null);
        }catch(Exception e){
            bot.getLogger().error("La commande "+command+" ne s'est pas exécutée à cause d'un problème sur la méthode "+((SimpleCommand)object[0]).getMethod().getName()+" qui ne s'est pas correctement exécutée.");
            bot.getErrorHandler().handleException(e);
        }
    }
    ...
    public boolean discordCommandUser(User user, String command, Message message){
        ...
        try{
            ...
            bot.getLogger().error("La methode "+((SimpleCommand)object[0]).getMethod().getName()+" n'est pas correctement initialisé. ("+e.getMessage()+")");
            bot.getErrorHandler().handleException(e);
        }
        return true;
    }
